...
class Tool {
 public:
  ...
  /** 
   * @brief Subclasses must fill in this factory method to define the mask for the
   specific tool they implement. */
  virtual FloatMatrix *CreateMask(float radius) = 0;

  /** 
   * @brief The controller (typically the GUI class) must call this method each time
  ...
  /** 
   * @brief The controller should call this function after calling StartStroke() to
  ...
  /** @brief The controller must call this method to complete the stroke when the
   user releases the mouse button. */
  void EndStroke(int x, int y);

  /** @brief This function will stamp the current mask onto the current buffer exactly
   one time at a location centered at (x,y) within the buffer. */
  void StampMaskOntoBuffer(int x, int y);

  /** @brief This function returns the current paint color used with the tool.  In
  ...
  /** @brief This function combines (i.e., blends) a paint_color with an underlying
  ...
  /**@brief Most tools only apply "paint" when they are moved on the canvas, but
